...
-- Collazione senza distinzione fra maiuscole e minuscole.
--
-- Serve alle ricerche che confrontano un nome o un indirizzo come lo scrive una
-- persona: su SQLite era `COLLATE NOCASE`, un valore predefinito del motore. Su
-- PostgreSQL non esiste per difetto, ma si puo' DICHIARARE -- ed e' preferibile
-- alla riscrittura delle interrogazioni con `lower()` su entrambi i lati, che
-- impedirebbe l'uso degli indici a meno di crearne di funzionali.
--
-- `deterministic = false` e' quello che permette a due testi diversi ("ABC" e
-- "abc") di risultare uguali; la richiede ICU per una collazione di questo tipo.
DO $$
BEGIN
...
-- Nessun PRAGMA: PostgreSQL applica sempre i vincoli di integrita' referenziale.
-- Sotto SQLite andavano accesi a ogni connessione, e dimenticarlo significava
-- perdere silenziosamente le cancellazioni a cascata.
...
CREATE TABLE IF NOT EXISTS probes (
    ...
    -- ISTANTANEA DELLA CONSOLE DELLA SONDA, consegnata col battito.
    --
    -- Perche' si conserva invece di chiederla quando serve: la sonda vive nella rete
    -- del cliente e parla solo in uscita -- NAT e firewall -- quindi il server non
    -- puo' interrogarla. L'unico canale e' il battito, ogni quindici secondi: lo
    -- stato arriva con quello, gia' pronto. L'istante non e' un dettaglio ma la
    -- parte onesta: dice di QUANDO e' cio' che si sta guardando.
    console_json          TEXT,
    console_at            TEXT,
...
CREATE TABLE IF NOT EXISTS subnets (
    ...
    -- RETE SENZA FILI. Non e' un'etichetta descrittiva: cambia il modo in cui la
    -- sonda la osserva. Su una rete Wi-Fi gli apparati si presentano e sparcono nel
    -- giro di minuti (un telefono che entra in una stanza, un portatile che si
    -- sospende), quindi una passata completa ogni tre giorni non li vede mai: qui la
    -- sonda esegue una ricognizione breve e frequente, e chi compare fa partire
    -- l'arricchimento subito (vedi presence.py nella sonda).
...
CREATE TABLE IF NOT EXISTS notifications (
    id          INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tenant_id   INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    -- Il vincolo verso check_incidents e' dichiarato in fondo al file, non qui:
    -- quella tabella e' definita DOPO questa e PostgreSQL risolve le chiavi esterne
    -- al momento della creazione (SQLite no, e per questo la dichiarazione in avanti
    -- era passata inosservata). Vedi "Vincoli differiti".
    incident_id INTEGER,
    event       TEXT    NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_ti_finding
    ON ti_findings(tenant_id, node_id, COALESCE(port_id, 0), kind,
                   COALESCE(cve_id, ''), COALESCE(technique_id, ''));
...
CREATE TABLE IF NOT EXISTS node_web (
    ...
    body_hash    TEXT,
    body_bytes   INTEGER,
    -- IMPRONTE DI SOMIGLIANZA. Non descrivono il nodo: servono a riconoscere che due
    -- nodi diversi sono la STESSA cosa. L'icona di un apparato e' un file che il
    -- costruttore mette nel firmware, quindi tutte le stampanti di quel modello la
    -- servono identica; l'insieme dei NOMI delle intestazioni HTTP e' l'impronta del
    -- programma che risponde. Sono l'unico indizio quando la pagina non dice nulla --
    -- un 401 nudo, una pagina di accesso senza titolo -- che e' il caso della maggior
    -- parte degli apparati incorporati. Dei nomi si conserva l'elenco, dei valori
    -- nulla: i valori possono contenere dati dell'apparato, i nomi no.
    favicon_hash TEXT,
    favicon_bytes INTEGER,
    favicon_path TEXT,
    headers_hash TEXT,
    headers_names TEXT,
...
-- Il riconoscimento per somiglianza cerca i nodi con la stessa impronta: senza questi
-- indici sarebbe una scansione della tabella per ogni nodo dell'inventario.
CREATE INDEX IF NOT EXISTS ix_node_web_favicon
...
--
-- Il problema che risolve: su una rete Wi-Fi un indirizzo non identifica un
-- apparato. Il DHCP lo riassegna, il telefono che entra in una stanza ne prende uno
-- e domani un altro, e nell'inventario resta un nodo che "esiste" per un indirizzo
-- che intanto e' di qualcun altro. Servono due cose distinte: CHI, e QUANDO era qui.
--
-- Una riga e' una PERMANENZA: un apparato riconosciuto, su un indirizzo, da un
-- istante a un altro. La permanenza si chiude e se ne apre un'altra quando
-- l'apparato ricompare su un indirizzo diverso, oppure dopo un'assenza abbastanza
-- lunga da non essere piu' la stessa visita.
--
-- `identity_source` dichiara SU CHE COSA e' stato riconosciuto, perche' la certezza
-- e' diversa: 'mac' e 'serial' identificano l'apparato, 'hostname' e' probabile,
-- 'address' vuol dire che non si ha un'identita' stabile e la permanenza riguarda
-- l'indirizzo. Non si finge di sapere chi era: si dichiara che non si sa.
CREATE TABLE IF NOT EXISTS presence_sessions (
    id           INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tenant_id    INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    subnet_id    INTEGER REFERENCES subnets(id) ON DELETE SET NULL,
    -- Il nodo dell'inventario, se ancora esiste: la permanenza sopravvive alla sua
    -- rimozione, perche' il fatto che quell'apparato sia stato qui resta vero.
    ...
    hostname     TEXT,
    device_label TEXT,
    -- Impronta DEBOLE del profilo (TTL, porte aperte, famiglia del sistema): non
    -- identifica un apparato -- mille telefoni uguali danno la stessa impronta -- ma
    -- se cambia SULLO STESSO INDIRIZZO dice che l'indirizzo e' passato a un altro
    -- apparato. E' l'unico modo di accorgersene quando il MAC non c'e'.
    profile_key  TEXT,
    first_seen_at TEXT   NOT NULL,
    last_seen_at TEXT    NOT NULL,
    sightings    INTEGER NOT NULL DEFAULT 1,
    -- Perche' la permanenza precedente si e' chiusa: serve a leggere lo storico
    -- ("tornato su un altro indirizzo" non e' la stessa cosa di "assente due ore").
    opened_reason TEXT,
    created_at   TEXT    NOT NULL
);
-- La ricerca della permanenza aperta di un'identita' e' l'interrogazione che si fa a
-- ogni avvistamento: senza indice sarebbe una scansione per ogni apparato visto.
CREATE INDEX IF NOT EXISTS ix_presenze_identita
    ON presence_sessions(tenant_id, identity_key, last_seen_at DESC);
CREATE INDEX IF NOT EXISTS ix_presenze_tempo
    ON presence_sessions(tenant_id, last_seen_at DESC);
CREATE INDEX IF NOT EXISTS ix_presenze_nodo ON presence_sessions(node_id);
CREATE INDEX IF NOT EXISTS ix_presenze_subnet ON presence_sessions(subnet_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_siem_alert_aperto
    ON siem_alerts(tenant_id, rule_code, COALESCE(group_value, ''))
...
-- Vincoli differiti
--
-- PostgreSQL risolve le chiavi esterne quando crea la tabella: un riferimento a
-- una tabella definita piu' avanti nel file non si puo' dichiarare in linea. Sotto
-- SQLite passava (i vincoli non venivano risolti in creazione), ed e' il motivo per
-- cui questo caso era rimasto inosservato.
--
-- Si dichiara qui, dove entrambe le tabelle esistono. La forma con il controllo su
-- pg_constraint serve all'idempotenza: lo schema viene riapplicato a ogni avvio,
-- come tutte le altre istruzioni "IF NOT EXISTS" di questo file.
-- ---------------------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint
                   WHERE conname = 'fk_notifications_incident') THEN
        ALTER TABLE notifications
            ADD CONSTRAINT fk_notifications_incident
            FOREIGN KEY (incident_id) REFERENCES check_incidents(id)
            ON DELETE SET NULL;
    END IF;
END $$;
...
-- Eventi SIEM
--
-- Stavano in un ARCHIVIO SEPARATO (un secondo file SQLite) perche' un flusso di
-- migliaia di righe al minuto contendeva le pagine alle interrogazioni della
-- console. Su PostgreSQL quella ragione non esiste -- i lettori non bloccano gli
-- scrittori -- e l'archivio separato costava una configurazione, una connessione e
-- una cancellazione manuale in piu': gli eventi non seguivano il tenant per
-- vincolo, e andavano rimossi a mano quando un tenant veniva eliminato.
--
-- Ora seguono il tenant come tutto il resto (ON DELETE CASCADE): un tenant che se
-- ne va non lascia i propri log, e non serve ricordarsi di cancellarli (GDPR).
-- ---------------------------------------------------------------------------
...
-- Il vincolo verso la sorgente serve indicizzato: `link_source` e la ricerca degli
-- host non dichiarati filtrano su questa colonna.
CREATE INDEX IF NOT EXISTS ix_siem_eventi_sorgente
    ON siem_events(source_id);

-- ====================================================================== --
-- IDS: rilevazioni conferite dalle sonde
-- ====================================================================== --
--
-- Le rilevazioni NON sostituiscono gli allarmi del SIEM: ci finiscono dentro. Ogni
-- rilevazione conferita diventa anche un evento (`event_kind = 'ids'`), cosi' le
-- regole a soglia, la correlazione e gli allarmi gia' scritti la vedono senza che
-- esista un secondo impianto -- che avrebbe voluto dire due posti in cui guardare e
-- due verita' sullo stesso fatto.
--
-- Qui si conserva la rilevazione COME TALE, con la sua prova e il suo conteggio: e'
-- cio' che si consegna a chi deve decidere, e ha una vita piu' lunga dell'evento.
CREATE TABLE IF NOT EXISTS ids_findings (
    id          INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tenant_id   INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    probe_id    INTEGER REFERENCES probes(id) ON DELETE SET NULL,
    ...
    ricevuta_at TEXT    NOT NULL,
    -- Chi l'ha presa in carico e quando: una rilevazione che nessuno ha guardato e
    -- una archiviata dopo verifica non sono la stessa cosa.
    decisa_da   INTEGER REFERENCES users(id) ON DELETE SET NULL,
    decisa_at   TEXT,
    nota        TEXT,
    UNIQUE (tenant_id, regola, soggetto)
);
CREATE INDEX IF NOT EXISTS ix_ids_findings_tempo
    ON ids_findings(tenant_id, ultima_at DESC);
CREATE INDEX IF NOT EXISTS ix_ids_findings_stato
    ON ids_findings(tenant_id, stato, gravita);

-- ====================================================================== --
-- Agenti di macchina
-- ====================================================================== --
--
-- Una macchina con l'agente e' un NODO dell'inventario visto da dentro. Non si
-- duplica il nodo: si tiene il legame, e dove manca si dichiara -- un agente su una
-- macchina che l'inventario non ha mai visto e' esso stesso un'informazione (una
-- macchina fuori dal perimetro dichiarato).
CREATE TABLE IF NOT EXISTS agent_hosts (
    ...
    registrato_at TEXT,
    ultimo_invio  TEXT,
    -- L'ULTIMO INVENTARIO della macchina: software installato, servizi, utenze,
    -- postura di sicurezza, porte in ascolto. E' uno STATO e si sovrascrive, mentre
    -- `agent_metrics` e' una serie e si accumula. Sulla stessa tabella avremmo tenuto
    -- 1.440 copie al giorno dello stesso elenco per poterne leggere una.
    inventario_json TEXT,
    inventario_at   TEXT,
    -- I gruppi che questa macchina non manda perche' chi l'ha installata li ha
    -- spenti: senza, la console leggerebbe l'assenza di un dato come un dato a zero.
    gruppi_spenti   TEXT,
    aggiornato_at TEXT    NOT NULL,
    UNIQUE (tenant_id, agent_uid)
);
CREATE INDEX IF NOT EXISTS ix_agent_hosts_visto
    ON agent_hosts(tenant_id, ultimo_invio DESC);

-- Le misure. Il dettaglio resta in JSON: l'insieme cambia con la versione
-- dell'agente e con il sistema operativo, e una colonna per misura sarebbe una
-- migrazione a ogni aggiunta. In colonna stanno i pochi valori su cui si filtra e si
-- disegna.
...
CREATE INDEX IF NOT EXISTS ix_agent_metrics_tempo
    ON agent_metrics(tenant_id, agent_uid, rilevato_at DESC);

-- Gli eventi riferiti dalle macchine. Vivono qui E nel SIEM: qui con il loro
-- dettaglio completo, li' normalizzati insieme a tutto il resto.
...
CREATE INDEX IF NOT EXISTS ix_agent_events_tempo
    ON agent_events(tenant_id, ricevuto_at DESC);
CREATE INDEX IF NOT EXISTS ix_agent_events_genere
    ON agent_events(tenant_id, genere, ricevuto_at DESC);

-- Quanto occupa l'archivio, misurato una volta al giorno. Una dimensione da sola non
-- risponde alla domanda che ci si fa guardandola -- "durera'?" -- e per rispondere
-- serve una SECONDA misura piu' tardi: la crescita e' una differenza, non un numero.
--
-- SENZA `tenant_id`, ed e' l'eccezione che conferma la regola: l'archivio e' uno solo
-- per tutti i tenant, la sua dimensione non e' attribuibile a nessuno di essi, e
-- spezzarla per tenant vorrebbe dire inventare una ripartizione. Riguarda
-- l'amministratore di sistema, come le copie e la conservazione.
...
CREATE INDEX IF NOT EXISTS ix_storage_samples_tempo
    ON storage_samples(measured_at DESC);

-- ---------------------------------------------------------------------------
-- Di chi e' un indirizzo pubblico: nome della rete, titolare, paese.
--
-- SENZA `tenant_id`, come i cataloghi della threat intelligence e per la stessa
-- ragione: che `203.0.113.0/24` sia di un certo operatore e' un fatto di internet,
-- non un dato di un cliente. Ripeterlo per tenant significherebbe interrogare N
-- volte lo stesso registro pubblico per ottenere N volte la stessa risposta.
--
-- SI CONSERVA L'INTERVALLO, NON L'INDIRIZZO: RDAP non risponde "questo indirizzo e'
-- di X" ma "l'intervallo da A a B e' di X". Una sola interrogazione copre cosi'
-- tutti gli indirizzi di quella rete -- spesso migliaia -- ed e' la ragione per cui
-- questa cache regge il traffico di un SIEM invece di soffocarlo.
--
-- Gli estremi stanno in ESADECIMALE a lunghezza fissa (8 caratteri per IPv4, 32 per
-- IPv6): un IPv6 e' un numero a 128 bit, che nessun intero di PostgreSQL o SQLite
-- contiene, mentre due esadecimali della stessa lunghezza si ordinano come i numeri
-- che rappresentano. Portabile su entrambi gli archivi senza tipi propri di nessuno.
CREATE TABLE IF NOT EXISTS net_ranges (
    id         INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_net_ranges_intervallo
    ON net_ranges(version, start_hex, end_hex);
-- L'indice che serve alla ricerca: "quale intervallo contiene questo indirizzo".
CREATE INDEX IF NOT EXISTS ix_net_ranges_ricerca
    ON net_ranges(version, start_hex, end_hex);

-- La coda che alimenta la cache da sola: ogni indirizzo pubblico che il prodotto
-- incontra finisce qui, e un thread di fondo lo risolve con calma.
--
-- Le righe NON si cancellano dopo la risoluzione: `attempts` ed `error` dicono che
-- ci si e' provati e perche' non c'e' un nome, che e' un'informazione diversa da
-- "non ci ha mai pensato nessuno". Senza, si riproverebbe all'infinito su indirizzi
-- che nessun registro conosce.
CREATE TABLE IF NOT EXISTS net_lookups (
    ip            TEXT PRIMARY KEY,
    first_seen_at TEXT NOT NULL,
    last_seen_at  TEXT NOT NULL,
    attempts      INTEGER NOT NULL DEFAULT 0,
...
-- I piu' visti di recente per primi: se la coda e' lunga, si risolve prima cio' che
-- qualcuno sta effettivamente guardando.
CREATE INDEX IF NOT EXISTS ix_net_lookups_coda
    ON net_lookups(resolved_at, next_try_at, last_seen_at DESC);

-- ---------------------------------------------------------------------------
-- Chi ha fatto una scheda di rete, dal prefisso del MAC (registro IEEE).
--
-- SENZA `tenant_id`, come gli altri cataloghi: che il prefisso `803F5D` sia di una
-- certa azienda e' un fatto del registro mondiale, non un dato di un cliente.
--
-- LA TABELLA E' LA CACHE. A differenza delle reti pubbliche non si interroga nessuno
-- per singolo indirizzo: il registro e' un file, si legge tutto in una volta e si
-- tiene qui. Un MAC nuovo non costa nessuna richiesta, e' una ricerca su un prefisso.
--
-- I PREFISSI NON SONO TUTTI LUNGHI UGUALE: lo IEEE assegna blocchi grandi (MA-L, 24
-- bit = 6 cifre), medi (MA-M, 28 = 7) e piccoli (MA-S, 36 = 9). Un blocco piccolo e'
-- condiviso fra molte aziende, quindi si cerca dal prefisso piu' lungo al piu' corto:
-- fermarsi ai primi tre byte darebbe il nome di chi ha rivenduto il blocco.
CREATE TABLE IF NOT EXISTS mac_vendors (
    prefix      TEXT PRIMARY KEY,
    vendor      TEXT NOT NULL,
    block       TEXT NOT NULL DEFAULT 'MA-L',
    imported_at TEXT NOT NULL
);
