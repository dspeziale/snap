...
def ingest_batch(collector: dict, righe: list) -> dict:
    ...
    for grezza in righe:
        ...
        if not testo.strip():
            continue
        testo = testo[:MAX_MESSAGE_BYTES]

        # Prima si prova ad attribuire, cosi' la famiglia della sorgente guida il
        # riconoscimento (le firme giuste per prime).
        host = host_trasporto
        sorgente = None
        if host and host in per_host:
            sorgente = per_host[host]
        evento = parsers.classify(testo, kind_dichiarato or (sorgente or {}).get("kind", ""))
        host = host or evento.get("host")
        ip = ip_trasporto or evento.get("src_ip")

        if sorgente is None and host and host in per_host:
            sorgente = per_host[host]
        ...
        riga = {
            ...
            "extra_json": store.normalizza_extra(evento.get("_extra")),
        }
        da_scrivere.append(riga)
        if sorgente:
            conteggi_sorgente[sorgente["id"]] = conteggi_sorgente.get(sorgente["id"], 0) + 1
